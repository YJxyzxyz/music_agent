...
def _save_state(songs: list[dict], caption: str, scene: str = "default") -> None:
    STATE_FILE.write_text(
        json.dumps(
            {"last_push_date": _today(), "songs": songs, "caption": caption, "scene": scene},
            ensure_ascii=False,
            indent=2,
        ),
        encoding="utf-8",
...
def run_once(force: bool = False, scene: str | None = None) -> dict:
    ...
    try:
        ...
        _save_state(songs, caption, scene)
        append_history(songs, caption, scene=scene)
        journal_path = write_journal(songs, caption, scene=scene)
